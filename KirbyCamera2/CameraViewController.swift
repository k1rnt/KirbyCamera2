...
class CameraViewController: UIViewController, AVCaptureVideoDataOutputSampleBufferDelegate, UIGestureRecognizerDelegate {
    ...
    @IBAction func wbShutter(_ sender: Any) {
        customSound()
        takeStillPicture()
...
//        closeButton.center = CGPoint(x: (underView.frame.size.width+wdBtn.center.x+wdBtn.frame.size.width/2)/2, y: underView.frame.size.height/2)
